...
<body>
            ...
            <table id="tablo" class="table">
                ...
                <tbody>
                    ...
                    if ($_POST) {
                        ...
                        $query = "SELECT * FROM log_isik WHERE log_acik BETWEEN '$log_acik' AND '$log_kapali' ";
    ...
    <div class="container2">
        <h3>Işığın Açılacağı Tarihi Seçiniz:</h3>
        <form action="" method="POST">
            <div class="form-group">
                ...
                <button type="submit" class="btn left btn-danger" name="gonder2">Işığı Kapat</button>
            </div>
        ...
        $sontarih = $db->query("SELECT * FROM isik_zaman WHERE id=1")->fetch(PDO::FETCH_ASSOC);
        ...
        $t = strtotime($bitistarih) - strtotime($baslangictarih);
            ...
            if ($t < 1) {
                ...
                $sonuc5 = $db->exec("UPDATE isik_zaman SET tarih = '2033-06-01T08:30' WHERE id = 1");
                echo "<br> Işık Açıldı";
                $sonuc = $db->exec("UPDATE statusled SET Stat = '1' WHERE id = 1");
                date_default_timezone_set('Europe/Istanbul');

                $tarihlog = date('Y.m.d H:i:s');
             
                $log1 = $db->query("INSERT INTO log_isik(log_acik) VALUES('$tarihlog')");
            } else {
                echo "<br>";
        ...
        if (isset($_POST['gonder'])) {
            $saat = $_POST['saat'];
            $sonuc2 = $db->exec("UPDATE isik_zaman SET tarih = '$saat' WHERE id = 1");
            header("Refresh: 0;");
        }
        ?>
      <?php
        if (isset($_POST['gonder2'])) {
            $sonuc3 = $db->exec("UPDATE statusled SET Stat = '0' WHERE id = 1");
            date_default_timezone_set('Europe/Istanbul');
            $tarihlog = date('Y.m.d H:i:s');
            $log2 = $db->query("UPDATE log_isik SET log_kapali='$tarihlog' WHERE log_kapali=0");
            header("Refresh: 0;");
        }
        ?>
    </div>
